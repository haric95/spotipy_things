...
def blend_artworks(listofimages):
    w,h = Image.open(listofimages[0]).size
    full_arr = np.zeros((h,w,3), np.float)
    N = len(listofimages)
    for i in listofimages:
         imarr = np.array(Image.open(i), dtype = np.uint8)
         full_arr = full_arr+imarr/N
    full_arr=np.array(np.round(full_arr),dtype=np.uint8)
    out = Image.fromarray(full_arr, mode="RGB")
    out.save("Blend.png")

blend_artworks(listofimages)